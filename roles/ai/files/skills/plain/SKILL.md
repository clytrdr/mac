...
This skill covers three kinds of target text:
...
```
/plain path/to/doc.md path/to/page.html path/to/module.py
```
...
5. Rewrite the target text in place with `Edit` following the English Writing Style rules above.
...
- **Never break structure.** In Markdown, do not change code blocks, inline code, link URLs, image paths, tables layout, or frontmatter keys. In HTML, do not change tags, attributes, URLs, `<script>`, `<style>`, or `<pre>`/`<code>` blocks.
